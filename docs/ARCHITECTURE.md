...
### 3.3 Поставка және предзаказ (2-кезең)

```
PLANNED ──«Открыть предзаказ»──► PREORDER_OPEN ──«В пути»──► IN_TRANSIT ──«Поставка пришла»──► ARRIVED ──► COMPLETED
   └──────────────────«В пути»──────────────────► IN_TRANSIT
PLANNED / PREORDER_OPEN / IN_TRANSIT ──► CANCELLED (белсенді предзаказдар да болдырылмайды)
```

* Бот предзаказ қабылдай ма — `Supply.preorderOpen` өрісі. Предзаказды PLANNED немесе IN_TRANSIT кезінде ашуға болады (жолда тұрған поставкаға да).
* Предзаказ = `PreOrder` + `PreOrderItem`. Қалдыққа (physical/reserved/available) **ешқандай** әсер етпейді.
* Позиция бойынша предзаказ лимиті (`SupplyItem.preorderLimit`) — қатар жасалған предзаказдар лимиттен аспайды (поставка жолы құлыпталады).
* «Поставка пришла»: админ әр позиция бойынша **нақты келген** санды енгізеді (міндетті, 0 болуы мүмкін). Қалдыққа тек сол сан қосылады, «Заказано» автоматты қосылмайды. Қайта басу мүмкін емес.
* Поставка келгеннен кейін менеджер предзаказды тапсырысқа айналдырады: санды азайта алады (толық келмесе), нәтижесінде `Order` (source=PREORDER) CONFIRMED болып жасалады → қалдықтан бронь. Қалдық жетпесе — ештеңе өзгермейді.
* Құжат нөмірлері: TF-000001 (заказ), PO-000001 (предзаказ), MB-000001 (монобукет) — базалық sequence, қайталанбайды.

### 3.4 Монобукет (1-нұсқа)
...
MonobouquetRequest (NEW) → Admin panel + Telegram: «💐 Новый запрос на монобукет»
   ↓
Менеджер клиентпен байланысты (CONTACTED), бағаны қолмен қояды
   ↓
Баға мен детальдар келісілді (CONFIRMED — бағасыз растауға болмайды)
   ↓
Дайын, берілді (COMPLETED)        немесе кез келген кезеңде → CANCELLED
```
...
* Клиенттің комментарийі (`comment`) тек оқылады; менеджердің ішкі жазбасы бөлек өрісте (`managerNote`) сақталады.
...
| 2 | Admin: Товары, Остатки (себептермен), Цены (тарихпен), Поставки + предзаказ + приход, Монобукеты, Настройки, Dashboard (кезеңдер), фото (Vercel Blob) | Каталогты толтыра аласыз | ✅ |
